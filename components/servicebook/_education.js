export function buildServiceBookEducation(data) {
  let a = data[0].EmpEducationDetail;

  const t1_present = a.map((edu) => ({
    Course: edu.Education,
    Institution: edu.Institution,
    Year: edu.year,
    Percentage: edu.MarksPercentage,
  }));

  let html = `
  ${buildSubtable("Educational Details", t1_present)}
  `;
  return html;
}
